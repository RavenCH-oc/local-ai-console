...
Each persisted or configurable contract has a `schema_version` and a stable machine `id` where it represents a named domain object. Existing Phase 0C examples use `1.0.0`; additive Phase 1B-3 contract types use `1.1.0`, and the validator accepts both. Display names are presentation data, never primary identity. Cross-contract references use stable IDs such as `preferred_model_profile_id` rather than copied objects, display names, or filesystem paths. Knowledge namespaces use a separate hierarchical namespace ID such as `comfyui/example`.
