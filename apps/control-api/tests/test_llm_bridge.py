...
class LlamaCppClientTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_generation_structured_output_and_provider_token_count_are_mapped(self) -> None:
        ...
        shape = client.safe_request_shape(request, stream=False)
        self.assertEqual(shape["model_field"], "configured_default")
        self.assertEqual(shape["message_content_types"], ["str"])
        self.assertEqual(shape["chat_template_kwargs"], {"enable_thinking": True})
        self.assertEqual(shape["structured_output_field"], True)
        self.assertNotIn("Test prompt", str(shape))
        self.assertNotIn("model-alias", str(shape))
